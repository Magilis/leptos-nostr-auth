...
impl ModalStep {
    fn title(&self) -> &'static str {
        match self {
            ModalStep::MethodSelect => "Connect to Nostr",
            ModalStep::Extension => "Browser Extension",
            ModalStep::Bunker => "Nostr Connect",
            ModalStep::Passkey => "Passkey",
            ModalStep::ReadOnly => "Read-Only Access",
            ModalStep::Ncryptsec => "Encrypted Key",
            #[cfg(feature = "insecure_nsec_input")]
            ModalStep::RawNsecWarn | ModalStep::RawNsec => "Secret Key",
        }
    }
}
...
fn icon_amber() -> impl IntoView {
    view! { <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" style="color:#f97316"><path d="M21 2l-2 2m-7.61 7.61a5.5 5.5 0 1 1-7.778 7.778 5.5 5.5 0 0 1 7.777-7.777zm0 0L15.5 7.5m0 0l3 3L22 7l-3-3m-3.5 3.5L19 4"/></svg> }
}
    ...
    let connect = move || {
        ...
        spawn_local(async move {
            match BunkerSession::connect(&uri_val, timeout).await {
                Ok(session) => {
                    set_loading.set(false);
                    on_auth.run(AuthResult::Bunker(session));
                }
                Err(e) => {
                    set_error.set(Some(e.to_string()));
                    set_loading.set(false);
